...
from utilstf import get_spectrogram, find_zeros_of_spectrogram_2, get_round_window
...
def compute_S0(radius, statistic = None, Sm = None):
    ...
    if statistic == 'L':
        ...
        Lrho_gaf = np.sqrt(Krho_gaf/np.pi)   # Theoretical value.
        return Lrho_gaf

    if statistic != 'L': # For other statistics, use the mean of the simulated.
        if Sm is not None:
...
def compute_mc_sim(signal,
    ...
    pos_exp = find_zeros_of_spectrogram_2(stf)/T
    simulation_pos = list()
    ...
    for i in range(MC_reps):   
        ...
        pos = find_zeros_of_spectrogram_2(stf)/T    
